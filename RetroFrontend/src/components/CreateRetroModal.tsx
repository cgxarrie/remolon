                ...
                <div className="p-6 space-y-4">
                    ...
                    {selectedOrganizationId && (
                        <div>
                            ...
                            <div className="border border-slate-200 dark:border-slate-700 rounded-lg max-h-40 overflow-y-auto divide-y divide-slate-100">
                                {managersQuery.isPending && (
                                    <p className="px-3 py-4 text-sm text-slate-500 dark:text-slate-400">Loading managers…</p>
                                )}
                                {managersQuery.isError && (
                                    <p className="px-3 py-4 text-sm text-red-600 dark:text-red-400">Failed to load managers.</p>
                                )}
                                ...
                                {managersQuery.data?.map((manager) => (
                                    <label key={manager.id} className="flex items-center gap-3 px-3 py-2 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 cursor-pointer">
                                        <input
                                            type="checkbox"
                                        ...
                                        <span className="min-w-0">
                                            <span className="block text-sm font-medium text-slate-800 dark:text-slate-100 truncate">{manager.nickname}</span>
                                            <span className="block text-xs text-slate-500 dark:text-slate-400 truncate">{manager.email}</span>
                                        </span>
                                    </label>
                                ))}
                            </div>
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">At least one manager must be assigned.</p>
                        </div>
                    )}
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Columns</label>
                        <ul className="space-y-2 mb-2">
                            {columns.map((col, i) => (
                                <li key={i} className="flex items-center gap-2">
                                    <span className="flex-1 text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-md px-3 py-1.5">
                                        {col}
                                    </span>
                                    <button
                                        onClick={() => removeColumn(i)}
                                        className="text-red-400 hover:text-red-600 dark:hover:text-red-400 text-sm"
                                    >
                                        ✕
                                    </button>
                                </li>
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 border border-slate-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                            />
                            <button
                                onClick={addColumn}
                                className="px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 rounded-md text-sm"
                            >
                                Add
                            </button>
                        </div>
